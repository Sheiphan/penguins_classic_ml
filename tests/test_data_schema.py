...
class TestPenguinRecord:
    ...
    def test_invalid_island(self):
        ...
        with pytest.raises(ValidationError):
            PenguinRecord(island="InvalidIsland", species="Adelie")

    def test_invalid_sex(self):
        """Test validation of invalid sex."""
        with pytest.raises(ValidationError):
            PenguinRecord(sex="UNKNOWN", species="Adelie")

    def test_invalid_species(self):
        """Test validation of invalid species."""
        with pytest.raises(ValidationError):
            PenguinRecord(species="InvalidSpecies")

    def test_species_name_parsing(self):
        """Test parsing of full species names."""
        record = PenguinRecord(species="Adelie Penguin (Pygoscelis adeliae)")
        assert record.species == "Adelie"

        record = PenguinRecord(species="Chinstrap penguin (Pygoscelis antarctica)")
        assert record.species == "Chinstrap"

    def test_negative_values(self):
        """Test validation of negative values."""
        with pytest.raises(ValidationError):
            PenguinRecord(bill_length_mm=-10.0)

        with pytest.raises(ValidationError):
            PenguinRecord(body_mass_g=-100.0)

    def test_invalid_year(self):
        """Test validation of invalid year."""
        with pytest.raises(ValidationError):
            PenguinRecord(year=2020)  # Outside valid range

        with pytest.raises(ValidationError):
            PenguinRecord(year=2000)  # Outside valid range
