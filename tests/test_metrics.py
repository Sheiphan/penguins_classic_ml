...
def sample_classification_data():
    ...
    for idx in error_indices:
        ...
        other_classes = [
            c for c in ["Adelie", "Chinstrap", "Gentoo"] if c != current_class
        ]
    ...
    for i, true_class in enumerate(y_true):
        ...
        y_proba[i][class_idx] = max(
            y_proba[i][class_idx], 0.4
        )  # Ensure some confidence
...
class TestModelMetrics:
    ...
    def test_compare_metrics(self):
        ...
        comparison_df = calculator.compare_metrics(
            [metrics1, metrics2, metrics3], ["Model_A", "Model_B", "Model_C"]
        )
...
class TestMetricsStorage:
    ...
    def test_compare_all_models(self, sample_classification_data):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (
                comparison_df.iloc[0]["accuracy"] >= comparison_df.iloc[1]["accuracy"]
            )
