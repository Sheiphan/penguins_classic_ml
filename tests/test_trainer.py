...
class TestModelTrainer:
    ...
    def test_create_model(self, trainer_with_temp_dir):
        ...
        model_config = ModelConfig(
            name="RandomForestClassifier", params={"n_estimators": 5, "max_depth": 3}
        )
    ...
    def test_train_with_provided_data(
        self, mock_loader, trainer_with_temp_dir, sample_data
    ):
    ...
    def test_train_without_data(self, trainer_with_temp_dir, sample_data):
        ...
        trainer.data_loader.train_test_split = Mock(
            return_value=(X[:80], X[80:], y[:80], y[80:])
        )
    ...
    def test_predict_proba_without_support(self, trainer_with_temp_dir, sample_data):
        ...
        with pytest.raises(
            ValueError, match="Model does not support probability predictions"
        ):
