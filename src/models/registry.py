...
class ModelRegistry:
    ...
    def load_model(self, model_id: str) -> Union[BaseEstimator, Pipeline]:
        ...
        if model_id not in self._registry["models"]:
            ...
            raise ValueError(f"Model {model_id} not found. Available: {available_models}")
    ...
    def load_latest_model(self) -> Optional[Union[BaseEstimator, Pipeline]]:
    ...
    def delete_model(self, model_id: str, delete_metrics: bool = True) -> None:
        ...
        if delete_metrics:
            try:
                metrics_files = list(self.metrics_storage.storage_dir.glob(f"{model_id}_metrics*.json"))
    ...
    def get_best_model_id(self, primary_metric: str = "f1_score") -> Optional[str]:
    ...
    def load_best_model(self, primary_metric: str = "f1_score") -> Optional[Union[BaseEstimator, Pipeline]]:
