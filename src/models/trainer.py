...
class ModelTrainer:
    ...
    def create_pipeline(self, model_config: ModelConfig | None = None) -> Pipeline:
    ...
    def train(
        ...
        logger.info(
            f"Training completed. Test accuracy: {test_metrics['accuracy']:.4f}"
        )
    ...
    def _prepare_param_grid(
        self, grid_config: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
    ...
    def _save_model_and_results(self, results: dict[str, Any], suffix: str = "") -> str:
    ...
    def predict(self, X: pd.DataFrame, model_id: str | None = None) -> pd.Series:
    ...
    def predict_proba(
        self, X: pd.DataFrame, model_id: str | None = None
    ) -> pd.DataFrame:
...
def train_model(
    config_path: str = "configs/experiment_default.yaml", save_model: bool = True
) -> dict[str, Any]:
...
def tune_model(
    config_path: str = "configs/experiment_default.yaml", save_best_model: bool = True
) -> dict[str, Any]:
