...
from .models.trainer import ModelTrainer, train_model, tune_model
...
def tune(config: str, output: Optional[str], no_save: bool) -> None:
    ...
    try:
        ...
        if experiment_config.model.tune is None:
            ...
            click.echo("Please add a 'tune' section to your model configuration.", err=True)
            sys.exit(1)
...
def serve(
    ...
    try:
        ...
        logger.info(f"Starting server on {serving_config.api.host}:{serving_config.api.port}")
...
def model_info(model_id: str, model_dir: str) -> None:
    ...
    try:
        ...
        if metrics_data:
            ...
            if data_info:
                click.echo("\nData Information:")
                click.echo(f"  Training samples: {data_info.get('train_size', 'Unknown')}")
                click.echo(f"  Test samples: {data_info.get('test_size', 'Unknown')}")
                click.echo(f"  Features: {len(data_info.get('features', []))}")
                click.echo(f"  Target classes: {', '.join(data_info.get('target_classes', []))}")
