...
class PenguinClassifierClient:
    ...
    def predict_single(self, penguin_data: dict[str, Any]) -> dict[str, Any]:
        ...
        Raises:
            RequestException: If request fails
        """
        response = self.session.post(f"{self.base_url}/predict", json=penguin_data)
        response.raise_for_status()
        return response.json()
    ...
    def predict_batch(self, penguin_instances: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        response = self.session.post(
            f"{self.base_url}/predict/batch", json={"instances": penguin_instances}
        )
        response.raise_for_status()
        return response.json()
...
def test_batch_predictions(client: PenguinClassifierClient) -> bool:
    ...
    try:
        ...
        if len(result["predictions"]) != len(instances):
            print(
                f"❌ Expected {len(instances)} predictions, got {len(result['predictions'])}"
            )
            return False
        ...
        for i, prediction in enumerate(result["predictions"]):
            ...
            if prediction["prediction"] not in ["Adelie", "Chinstrap", "Gentoo"]:
                print(
                    f"❌ Invalid prediction in result {i}: {prediction['prediction']}"
                )
                return False
    ...
    for case in error_cases:
        try:
            ...
            response = client.session.post(
                f"{client.base_url}/predict", json=case["data"]
            )
...
def test_performance(client: PenguinClassifierClient, num_requests: int = 10) -> bool:
    ...
    for i in range(num_requests):
        try:
            ...
            result = client.predict_single(test_penguin)  # noqa: F841
            end_time = time.time()
...
def main():
    ...
    if args.performance:
        tests.append(
            ("Performance", lambda: test_performance(client, args.num_requests))
        )
